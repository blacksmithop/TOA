...
import { getRecommendedMembers, type MemberRecommendation } from "@/lib/tornstats"
...
export default function CrimesList({
  ...
  const [showRecommendations, setShowRecommendations] = useState<Map<string, boolean>>(new Map())
  const [recommendations, setRecommendations] = useState<Map<string, MemberRecommendation[]>>(new Map())
  ...
  const [tornStatsApiKey, setTornStatsApiKey] = useState<string | null>(null)
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      ...
      const apiKey = localStorage.getItem('TORNSTATS_API_KEY')
  ...
  const handleRecommendClick = async (crimeId: number, crimeName: string, position: string, slotIdx: number) => {
    const recommendKey = `${crimeId}-${slotIdx}`
    
    if (loadingRecommendations.has(recommendKey)) return

    if (!tornStatsApiKey) {
      console.error('[v0] TornStats API key not found')
      return
    }

    setLoadingRecommendations(prev => new Set(prev).add(recommendKey))

    try {
      const recs = await getRecommendedMembers(tornStatsApiKey, crimeName, position, membersNotInOCSet, minPassRate)
      ...
      setRecommendations(prev => {
        const next = new Map(prev)
        next.set(recommendKey, recsWithNames)
        return next
      })

      setShowRecommendations(prev => {
        const next = new Map(prev)
        next.set(recommendKey, true)
        return next
      })
    } finally {
      setLoadingRecommendations(prev => {
        const next = new Set(prev)
        next.delete(recommendKey)
        return next
      })
    }
  }
                      ...
                      <div className="pt-1.5 border-t border-border/30">
                        ...
                        <div className="space-y-1">
                          {crime.slots.map((slot, idx) => {
                            ...
                            const recommendKey = `${crime.id}-${idx}`
                            const isLoadingRecs = loadingRecommendations.has(recommendKey)
                            const slotRecommendations = recommendations.get(recommendKey) || []
                            const showingRecs = showRecommendations.get(recommendKey) || false
                            ...
                            return (
                              <div key={idx} className="space-y-1">
                                    ...
                                    <div className="flex items-center gap-2 min-w-0 flex-1">
                                      ...
                                      {slot.user ? (
                                        ...
                                        <>
                                          ...
                                          {isOpenRecruitingSlot && tornStatsEnabled && (
                                            <button
                                              onClick={() => handleRecommendClick(crime.id, crime.name, slot.position, idx)}
                                            ...
                                            >
                                              {isLoadingRecs ? 'Loading...' : 'Recommend'}
                                            </button>
                                          )}
                                        </>
                                      )}
                                ...
                                {showingRecs && slotRecommendations.length > 0 && (
                                  <div className="ml-4 p-2 bg-blue-500/10 border border-blue-500/30 rounded text-xs">
                                    <div className="flex items-center justify-between mb-1">
                                      <p className="font-bold text-blue-400">Recommended Members ({slotRecommendations.length})</p>
                                      <button
                                        onClick={() => {
                                          setShowRecommendations(prev => {
                                            const next = new Map(prev)
                                            next.set(recommendKey, false)
                                            return next
                                          })
                                        }}
                                    ...
                                    <div className="space-y-0.5 max-h-32 overflow-y-auto">
                                      {slotRecommendations.map((rec, recIdx) => (
