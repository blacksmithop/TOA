...
export default function CrimesList({
  ...
  const copyToClipboard = (id: number) => {
    ...
    // Toast will be shown from parent dashboard component
  }
        ...
        {isExpanded && (
          <div className="mt-2 space-y-2 animate-in fade-in duration-200">
            ...
            {crimes.length === 0 ? (
              ...
              crimes.map((crime) => {
                const hasNavigation = !["Successful", "Expired"].includes(crime.status)
                const crimeAnchorId = hasNavigation ? `crime-${crime.id}` : undefined
